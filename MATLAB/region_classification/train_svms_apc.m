function models=train_svms_apc(imnames, feat_dir, region_meta_info, cachedir, run_name, varargin)
%random seed
rng(3);
...
% addOptional(p, 'to_train', [6,15], @isnumeric);
addOptional(p, 'to_train', OBJ_OF_INTEREST_APC, @isnumeric);
...
% TO DO: orignal work uses 2000. This is a parameter also to preprocess_mcg_candidates??

addOptional(p, 'retrain_limit', 200, @isscalar);
parse(p, varargin{:});
opts=p.Results;
...
for i=1:numel(imnames)
    ...
    for j=1:numel(to_train)
        ...
        if(islatent)
            ...
            ovs=region_meta_info.overlaps{i}(gtidx,:);
...
for j=1:numel(to_train)
    ...
    % -----------
    % Ahmed, please pay close attention to how negatives are selected
    % -----------
    
    % there are 1 + N regions in the variable region_meta_info.gt, where 1
    % is the groundtruth of the image, and N is the number of region
    % candidates (or proposals) computed using MCG.
    
    % This code section is to select regions (as negatives to SVM) from
    % image i. If the groundtruth object in the image is different from the
    % query object j, then the groundtruth region is selected. 
    ...
    nongt=find(region_meta_info.gt{i}==0); % find the index of region candidates/proposals computed from MCG
    gtidx=find(region_meta_info.gt{i}==to_train(j)); % find the index of regions in the current image i contain the same object as the query object j.
    if(~isempty(gtidx)) % if the image i contains the same object
        idx=find(max(region_meta_info.overlaps{i}(gtidx,:),[],1)<neg_ov_thresh);
        nongtidx=nongt(idx);
    else % if the image i doesn't contain the same object
        nongtidx=nongt;
    end
    ...
    % The key is a pair of the examined frame index (i.e., i) and the region index
    ...
    % Hao edited
    %-----------------
    if (~isempty(keys{j})) % if keys{j} not empty, check duplicates
        keep=~ismember(tmp_keys, keys{j}, 'rows');
    else % otherwise, keep all keys
        keep=~ismember(tmp_keys, [-1,-1], 'rows');
    end
